...
exports.ensureSubState = ensureSubState;
function ensureSubState(state, stateKey) {
    if (!stateKey) {
        return state;
    }

    var subState = state[stateKey];
    if (subState) {
        return subState;
    }
...
function validatePayload(validation, name, payload) {
    if (typeof validation === 'function') {
        if (!validation(payload)) {
            ...
            var errors = Object.keys(validation).map(function (k) {
                return {
                    key: k,
                    value: payload[k],
...
var _extractActions = __webpack_require__(10);

var _extractActions2 = _interopRequireDefault(_extractActions);

var _extractReducer = __webpack_require__(12);

var _extractReducer2 = _interopRequireDefault(_extractReducer);

var _extractMiddleware = __webpack_require__(11);

var _extractMiddleware2 = _interopRequireDefault(_extractMiddleware);
...
function extractActions(definitionMap, dispatch) {
    var actions = {};
    var start = new Date();

    var _loop = function _loop(k) {
        var definition = definitionMap[k];
        var actionNames = Object.keys(definition).concat(definition.$actionName).filter(function (k) {
            return k && k[0] !== '$';
        });
        ...
        actionNames.forEach(function (actionName) {
            ...
            var getPayload = definition[actionName] || definition.$getPayload;
    ...
    var count = Object.keys(actions).length - 1;
...
function getActionCaller(name, definition, actions, getPayload) {
    ...
    if (!getPayload) {
        getPayload = function getPayload(args) {
            return args;
        };
    }
    ...
    return function () {
        var action = {
            type: name,
            payload: getPayload.apply(undefined, arguments)
        };
        if ($meta) {
            action.meta = $meta;
        }
...
var _validatePayload = __webpack_require__(2);

var _validatePayload2 = _interopRequireDefault(_validatePayload);
...
function extractMiddleware(definitionMap, context, stateKey) {
    ...
    var filteredDefinitionMap = {};
    for (var k in definitionMap) {
        var _definitionMap$k = definitionMap[k],
            $before = _definitionMap$k.$before,
            $after = _definitionMap$k.$after,
            $stateKey = _definitionMap$k.$stateKey;
    ...
    var func = function func(store) {
        return function (next) {
            return function (action) {

                var definition = filteredDefinitionMap[action.type];
                if (!definition) return next(action);

                if (definition.$before) {
                    var state = store.getState();
                    state = (0, _util.ensureSubState)(state, stateKey);
                ...
                if (definition.$after) {
                    ...
                    _state = (0, _util.ensureSubState)(_state, stateKey);
                    definition.$after(context, _state, action.payload);
                }
            };
        };
    };
...
exports.default = extractReducer;
...
function extractReducer(definitionMap) {
    return function () {
        ...
        var definition = definitionMap[type];
        ...
        if (definition.$validation) {
            (0, _validatePayload2.default)(definition.$validation, type, payload);
        }

        state = (0, _util.ensureSubState)(state, definition.$stateKey);
